...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        # Forward pass
        layer1 = X @ W1 + b1                        # (N, hidden_size)
        layer1_sigmoid = self.sigmoid(layer1)       # (N, hidden_size)
        scores = layer1_sigmoid @ W2 + b2           # (N, num_classes)
        prob = self.softmax(scores)                 # (N, num_classes)
        ...
        # derivative of L to scores 
        d_scores = prob.copy()                          # (N, num_classes)
        d_scores[np.arange(N), y] -= 1                  # (N, num_classes)
        d_scores /= N                                   # (N, num_classes)
        
        # derivative of L to W2 and b2 
        d_W2 = layer1_sigmoid.T @ d_scores              # (hidden_size, num_classes)
        d_b2 = np.sum(d_scores, axis=0)      
        self.gradients['W2'] = d_W2
        self.gradients['b2'] = d_b2

        # derivative of L to layer1_sigmoid
        d_layer1_sigmoid = d_scores @ W2.T              # (N, hidden_size)
        
        # derivative of layer1_sigmoid to layer1
        d_layer1 = d_layer1_sigmoid * self.sigmoid_dev(layer1)  # (N, hidden_size)
       
        # derivative of L to W1 and b1
        d_W1 = X.T @ d_layer1                           # (input_size, hidden_size)
        d_b1 = np.sum(d_layer1, axis=0)
        self.gradients['W1'] = d_W1
        self.gradients['b1'] = d_b1
