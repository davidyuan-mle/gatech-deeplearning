...
class Linear:
    ...
    def backward(self, dout):
        ...
        #############################################################################
        self.dw = x.T @ dout    # (in_dim, N) x (N, out_dim) = (in_dim, out_dim)
        self.db = np.sum(dout, axis=0)
        self.dx = dout @ self.weight.T   # (N, out_dim) x (out_dim, in_dim) = (N, in_dim)
        self.dx = self.dx.reshape(self.raw_shape)
